...
export default function GetAllUsers({ data }) {
    ...
    const handleSearchInputChange = (event) => {
      ...
      const filteredData = data.filter((product) => {
        ...
        const matchesSearch =
          product.Firstname.toLowerCase().includes(searchValue.toLowerCase()) ||
          product.Lastname.toLowerCase().includes(searchValue.toLowerCase()) ||
    ...
    const perPage = 5;
      ...
      const handelBlock = (product) => {
        const url = `http://server-octoo-shop-production.up.railway.app/User/block/${product.Id}`;
      ...
      const handelUnblock = (product) => {
        const url = `http://server-octoo-shop-production.up.railway.app/User/unblock/${product.Id}`;
      ...
      const handelDelete = (product) => {
        axios
        .delete(`http://server-octoo-shop-production.up.railway.app/User/delete/${product.Id}`)
        .then((response) => {
          window.location.reload();
          setShowUser(true);
        })
          ...
          <div className="bg-white w-3/4 md:w-1/2 lg:w-1/3 rounded-lg overflow-hidden shadow-lg relative">
            ...
            {product.Blocked ? (
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div>
                <p className="font-bold text-gray-600">Dath of birth:</p>
                  <div className="text-sm">
                    {new Date(product.DOB).toLocaleDateString("en-US", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })}
  ...
  return (
      ...
      <table className="w-full">
                ...
                >
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="ml-3">
                        <p className="whitespace-no-wrap">{product.Firstname} {product.Lastname}</p>
    ...
    <div className="flex flex-col items-center border-t bg-white px-5 py-5 sm:flex-row sm:justify-between">
            <span className="text-xs text-gray-600 sm:text-sm"> Total <span className="text-indigo-500">{count}</span> active user found and <span className="text-red-500">{blockedCount}</span> blocked users. </span>
